        ...
        (savePath: string, peer: RTCPeer<T, TData>, fileNameInfo: FileInfo, fileRequest: FileRequest) => call(function* () {
            ...
            try {
                try {
                    ...
                    yield put(RtcActions.fileReceiveProgress({ fileRequest, totalBytes: fileNameInfo.size, downloadedBytes: 0, downloadedPct: 0 }))

                    for (; ;) {
                        const progTake = take(progressChannel)
                        const errTake = take(errorChannel)
                        const endTake = take(endChannel)
                        const result = yield race({ progress: progTake, error: errTake, end: endTake })
                        if (result.err) {
                            throw result.err
                        }
                        if (result.end) {
                        ...
                        if (result.progress) {
                            ...
                            if (newPct !== currentProgressPct) {
                                ...
                                yield put(RtcActions.fileReceiveProgress({ fileRequest, totalBytes: fileNameInfo.size, downloadedBytes: p.transferred, downloadedPct: currentProgressPct }))
                            }

                            if (p.remaining === 0) {
                                yield take(endChannel)
                                yield put(RtcActions.fileReceiveProgress({ fileRequest, totalBytes: fileNameInfo.size, downloadedBytes: p.transferred, downloadedPct: 100 }))
                                break;
                            }
                        }
                    }
