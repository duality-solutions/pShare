...
import { open, close } from "../rtcSaga";
...
export const copyFileToRTCPeer = <T extends string, TData extends string | Blob | ArrayBuffer | ArrayBufferView>(filePathInfo: FilePathInfo, peer: RTCPeer<T, TData>) => call(function* () {
    ...
    for (; ;) {
        ...
        const { bytesRead }: {
            bytesRead: number;
        } = yield call(() => read(fd, buffer, 0, amtToRead, totalCopied));
