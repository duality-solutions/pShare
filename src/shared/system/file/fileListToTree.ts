...
function isDownloadableFile(
    item: SharedFile | DownloadableFile
): item is DownloadableFile {
...
export function fileListToTree<T extends SharedFile | DownloadableFile>(
    list: T[]
): DirectoryEntry<T> {
    if (list.length === 0) {
        //throw {}
    ...
    const fileSegmentsList = bList.select(file => {
...
function fileSegmentsListToTree<T extends SharedFile | DownloadableFile>(
    ...
    const dirEntries: Enumerable<DirectoryEntry<T>> = dirs
        ...
        .select(g =>
            fileSegmentsListToTree(
                g.key,
                g.select(
                    ({ pathSegments: [, ...pathSegments], ...remaining }) => ({
                        ...remaining,
                        pathSegments,
                    })
                ),
                baseDir ? path.join(baseDir, g.key) : undefined
            )
        );
